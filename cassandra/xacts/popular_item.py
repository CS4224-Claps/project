def execute(session, input_arr, data_lines=[]):
    ...
    for o_id in range(d_next_o_id - num_orders, d_next_o_id):
        prepare_order = session.prepare(
            "SELECT O_ID, O_ENTRY_D, O_C_ID FROM Orders WHERE O_W_ID = ? AND O_D_ID = ? AND  O_ID = ?")
    ...
    for order in orders:
        prepare_customer = session.prepare(
            "SELECT C_FIRST, C_MIDDLE, C_LAST FROM Customer WHERE C_W_ID = ? AND C_D_ID = ? AND C_ID = ?")
    ...
    for order in orders:
        prepare_orderline = session.prepare(
            "SELECT OL_I_ID, I_NAME, OL_QUANTITY FROM OrderLine WHERE OL_W_ID = ? AND OL_D_ID = ? AND OL_O_ID = ?")
    ...
    for i, order in enumerate(orders):
        ...
        print("Customer: ", customers[i].c_first, customers[i].c_middle, customers[i].c_last)
        print("Popular items: ")
    ...
    for item_id, occurrences in popular_item_occurrences.items():
        print("  ", popular_item_id_names[item_id], f"{occurrences / num_orders * 100}%")

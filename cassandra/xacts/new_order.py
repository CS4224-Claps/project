...
def execute(session, input_arr, data_lines=[]):
    ...
    district = d_rows[0]
    o_id, d_tax, w_tax = int(
        district.d_next_o_id), float(district.d_tax), float(district.w_tax)
    ...
    session.execute(prepare_create_order.bind(
    ...
    for i in range(int(num_items)):
        ...
        s_rows = session.execute(prepare_stock.bind(
        ...
        stock = s_rows[0]
        s_quantity, s_ytd, s_order_cnt, s_remote_cnt, i_name, i_price = float(
            stock.s_quantity), float(stock.s_ytd), int(stock.s_order_cnt), int(stock.s_remote_cnt), stock.i_name, float(stock.i_price)
    ...
    customer = c_rows[0]
    c_last, c_credit, c_discount = customer.c_last, customer.c_credit, float(
        customer.c_discount)

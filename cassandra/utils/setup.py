...
LOAD_CUSTOMER = "COPY wholesale.Customer (C_W_ID, C_D_ID, C_ID, C_FIRST, C_MIDDLE, C_LAST, C_STREET_1, C_STREET_2, C_CITY, C_STATE, C_ZIP, C_PHONE, C_SINCE, C_CREDIT, C_CREDIT_LIM, C_DISCOUNT, C_BALANCE, C_YTD_PAYMENT, C_PAYMENT_CNT, C_DELIVERY_CNT, C_DATA) FROM '{}';"
...
LOAD_STOCK = "COPY wholesale.Stock (S_W_ID, S_I_ID, S_QUANTITY, S_YTD, S_ORDER_CNT, S_REMOTEj_CNT, S_DIST_01, S_DIST_02, S_DIST_03, S_DIST_04, S_DIST_05, S_DIST_06, S_DIST_07, S_DIST_08, S_DIST_09, S_DIST_10, S_DATA, I_PRICE, I_NAME) FROM '{}';"
...
def add_wtax_to_district(data_folder):
    add_attrs_to_table(data_folder, WAREHOUSE, DISTRICT, [0], [0], [7])
...
def add_iname_to_orderline(data_folder):
    add_attrs_to_table(data_folder, ITEM, ORDERLINE, [0], [4], [1])

def add_attrs_to_table(data_folder, from_csv, to_csv, from_pk_indices, to_pk_indices, attr_indices):
    """
    Creates a local copy (in TEMP folder) of the to_csv with additional attributes from the from_csv.
    """
    with open(os.path.join(data_folder, from_csv), 'r') as from_table, \
         open(os.path.join(data_folder, to_csv), 'r') as to_table, \
         open(os.path.join(TEMP, to_csv), 'w') as new_table:

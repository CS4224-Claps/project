...
def execute(conn, io_line, data_lines=[]):
    ...
    with conn.cursor() as cur:
        ...
        sql = """
            INSERT INTO Order_Read 
                (O_W_ID, O_D_ID, O_ID, O_C_ID, O_OL_CNT, O_ALL_LOCAL, O_ENTRY_D)
        ...
        sql = """
            INSERT INTO Order_Write 
                (O_W_ID, O_D_ID, O_ID, O_C_ID, O_CARRIER_ID)
                VALUES (%s, %s, %s, %s, %s)
        """
        cur.execute(
            sql, (w_id, d_id, o_id, c_id, None)
        )
        ...
        sql = """
            INSERT INTO OrderLine_Read 
                (OL_W_ID, OL_D_ID, OL_O_ID,
                OL_NUMBER, OL_I_ID, OL_AMOUNT,
                OL_SUPPLY_W_ID, OL_QUANTITY, OL_DIST_INFO)
                VALUES
        """

        items_values = ",".join(
            cur.mogrify("(%s,%s,%s,%s,%s,%s,%s,%s,%s)", item).decode("utf-8")
            for item in ordered_items_read
        )
        cur.execute(sql + items_values)

        sql = """
            INSERT INTO OrderLine_Write 
                (OL_W_ID, OL_D_ID, OL_O_ID, OL_NUMBER, OL_I_ID, OL_DELIVERY_D)
                VALUES 
        """
        items_values = ",".join(
            cur.mogrify("(%s,%s,%s,%s,%s,%s)", item).decode("utf-8")
            for item in ordered_items_write
        )
        cur.execute(sql + items_values)     

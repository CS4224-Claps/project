...
def execute(conn, io_line):
    ...
    with conn.cursor() as cur:
        ...
        o_id, o_entry_d = cur.fetchone()

        get_carrier_id = """
            SELECT O_CARRIER_ID 
                FROM Order_Write 
                WHERE O_W_ID = %s AND O_D_ID = %s AND O_ID = %s AND O_C_ID = %s
                LIMIT 1; 
        """
        cur.execute(get_carrier_id, (w_id, d_id, o_id, c_id))
        o_carrier_id, = cur.fetchone()

        get_order_items = """
            SELECT OL_I_ID, OL_SUPPLY_W_ID, OL_QUANTITY, OL_AMOUNT, OL_DELIVERY_D
            FROM OrderLine_Read NATURAL JOIN OrderLine_Write 
